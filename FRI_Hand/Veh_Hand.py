...
hand_sheet1 = pe.get_sheet (file_name= "FRI_Veh_Hand.xlsx")

# Create a TCP/IP socket
veh_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

# Connect the socket to the server's address and port
server_address = ('192.168.20.200', 6003) # 192.168.20.200
conn_flag = 0
...
VID = sys.argv[1] # "KK0WPJP" # sys.argv[1]
VIDnew = sys.argv[2] # "UZ8KYV1" # sys.argv[2]

auth_flag = 0
reg_flag = 0

for row in reg_sheet1 : # Get ( f(x), VID, VPR, f(w^i), f*(w^2i), Veh_Reg_comp_time )
    if row[1] == VID :
        fx_list = row[0]
        VPR = row[2]
...
for row in auth_sheet1 :
    if row[1] == VIDnew :
        S_auth = row[2]
        auth_flag = 1
        print ("  VIDnew : ", VIDnew, " found from init auth...")
        break 

if reg_flag == 1 and auth_flag == 1:
    print ("Fetched all details from Excels")
    ...
    VIDnew_Hand_req_S_auth_T1 = VIDnew + "&"+ "H1"+ "&"+ S_auth+ "&"+ str(T1)

    veh_socket.send (VIDnew_Hand_req_S_auth_T1.encode('utf')) 
    ti_VPR_i_val_T1 = veh_socket.recv(1024).decode()  # receive (challenge ti, i) from RSU2

    start1_comp_time = time.time ()
    ti_VPR_i_val_T1 = [i for i in ti_VPR_i_val_T1.split('&')]

    ti = int(ti_VPR_i_val_T1[0])
    R_hand = ti_VPR_i_val_T1[1]
    i_val = int(ti_VPR_i_val_T1[2])
    T2 = float (ti_VPR_i_val_T1[3])

    if get_timestamp () - T2 < 4 : #and VPR == VPR_star :
        ...
        proof_pi_T2 = ABC_proof + "&"+ auth_path_for_ti + "&"+ R_hand + "&"+ str(T2)
        print ("Sending Response to Challenge ....")

        end1_comp_time = time.time ()
        comp_time = end1_comp_time - start1_comp_time

        veh_socket.send (proof_pi_T2.encode('utf')) 
        VIDnew_Hand_status_S_hand = veh_socket.recv(1024).decode() # Auth status from RSU2

        VIDnew_Hand_status_S_hand = [i for i in VIDnew_Hand_status_S_hand.split('&')]
        VIDnew = VIDnew_Hand_status_S_hand[0] # VIDnew + "&"+ "S" + "&"+ str(S_auth)
        S_auth = VIDnew_Hand_status_S_hand[2]

        if VIDnew_Hand_status_S_hand[1] == "S" :
            print ("Hand Auth Success")
            '''
            ...
            '''
            print ("Hand comp time at Veh : ", comp_time)
            hand_sheet1.row += [ VIDnew, S_auth, comp_time ]
            hand_sheet1.save_as ("FRI_Veh_Hand.xlsx")
        else :
            print ("Hand Auth Failed")
    else :
        print ("T1 timestamp check failed ...")
else :
    print ("Reg Details not found in Sheet")
