...
veh_id = sys.argv[1] #"H0Y6MEP" # sys.argv[1]
veh_pswd = sys.argv[2] #"t1" 
n = 103

auth_sheet = pe.get_sheet(file_name="Conf_Veh_auth_comp_time.xlsx")

IPW = sha256(veh_id.encode('utf-8') + veh_pswd.encode('utf-8')).hexdigest()

reg_sheet = pe.get_sheet (file_name = "Conf_Reg_veh_time.xlsx")
row_flag = 0
...
if row_flag == 1 :
    ...
    if values[0] == str(auth_init_r) :
        ...
        auth_sheet.row += [veh_id, recv2[0], recv2[1], recv2[2], veh_comp_time]
        auth_sheet.save_as ("Conf_Veh_auth_comp_time.xlsx")
        print ("===========+++++++++ Veh Auth Done ++++++++++===============\n\n")
        veh_socket.close() 
